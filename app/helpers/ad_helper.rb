...
module AdHelper
  ...
  def shinka_ad
    return smaato_mxit_ad unless shinka_ads_enabled?
    ad = get_shinka_ad
    return "" if ad.empty?
  ...
  def smaato_mxit_ad
    smaato_ad("&format=txt&formatstrict=true").gsub("href=","onclick='window.open(this.href); return false;' href=").html_safe
  end
  ...
  def get_shinka_ad
    begin
      ...
      Settings.shinka_disabled_until = 20.seconds.from_now # disable for 20 seconds
      return {}
    rescue Exception => e
      ENV['AIRBRAKE_API_KEY'].present? ? notify_airbrake(e) : Rails.logger.error(e.message)
